...
async function main() {
  ...
  const dirs = await fs.promises.readdir(componentLocaleRoot, { withFileTypes: true })
  ...
  await fs.promises.writeFile(path.resolve(localeOutput, 'lang.js'), JSON.stringify(languages), {
    encoding: 'utf-8',
  })
...
async function traverseDir(dir, paths, targetPath) {
  ...
  await Promise.all(contents.map(async c => {
    if (c.isDirectory()) {
      await fs.promises.mkdir(path.resolve(targetPath, c.name), { recursive: true })

      return traverseDir(path.resolve(dir, c.name), paths.map(p => {
        return {
          ...p,
