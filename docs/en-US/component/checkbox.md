...
| value ^(2.6.0)                 | value of the Checkbox when used inside a `checkbox-group`                                                                                                      | ^[enum]`'string'\|'number'\|'boolean'\|'object'` | —       |
| label                          | label of the Checkbox when used inside a `checkbox-group`. If there's no value, `label` will act as `value`                                                    | ^[enum]`'string'\|'number'\|'boolean'\|'object'` | —       |
