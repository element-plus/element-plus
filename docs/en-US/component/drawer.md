...
| size                       | drawer's size, if Drawer is horizontal mode, it affects the width property, otherwise it affects the height property, when size is `number` type, it describes the size by unit of pixels; when size is `string` type, it should be used with `x%` notation, otherwise it will be interpreted to pixel unit | ^[number] / ^[string]                                                                                                                                                                          | 30%     |
| title                      |drawer's title, can also be set by named slot, detailed descriptions can be found in the slot section                                                                                                                                                                                                           | ^[string]                                                                                                                                                                                      | —       |
