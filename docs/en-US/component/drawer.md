...
| resize-start     | Triggered when resizing starts (when `resizable` is enabled) | ^[Function]`(event: MouseEvent) => void`               |
| resize           | Triggered while resizing (when `resizable` is enabled)       | ^[Function]`(event: MouseEvent, size: number) => void` |
| resize-end       | Triggered when resizing ends (when `resizable` is enabled)   | ^[Function]`(event: MouseEvent, size: number) => void` |
