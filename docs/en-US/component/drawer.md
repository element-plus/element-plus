...
:::demo Set `resizable` to `true` to resize. Use `min-size` and `max-size` ^(2.13.1) to constrain the drawer's dimensions. When `min-size` is not set, the drawer cannot be resized smaller than its initial `size`. When `max-size` is not set, the drawer cannot exceed the viewport size.
...
| min-size ^(2.13.1)         | minimum size of drawer, accepts number (px) or string with `x%` / `xpx` notation. If not set, defaults to the value of `size` prop, so the drawer cannot be resized smaller than its initial size. If `size` is also not set, defaults to 200px                                                              | ^[number] / ^[string]                                                                                                                                                                          | —       |
| max-size ^(2.13.1)         | maximum size of drawer, accepts number (px) or string with `x%` / `xpx` notation. If not set, defaults to the full viewport size                                                                                                                                                                             | ^[number] / ^[string]                                                                                                                                                                          | —       |
