...
:::demo Set `resizable` to `true` to resize. Use `resizable` with an object ^(2.13.1) to constrain the drawer's dimensions. When `minSize` is not set, the drawer cannot be resized smaller than its initial `size`. When `maxSize` is not set, the drawer cannot exceed the viewport size.
...
| resizable ^(2.11.0)        | enable resizable feature for Drawer. Use an object ^(2.13.1) to configure `minSize` and `maxSize` in pixels                                                                                                                                                                                                  | ^[boolean] / ^[object]`{ minSize?: number, maxSize?: number }`                                                                                                                                 | false   |
