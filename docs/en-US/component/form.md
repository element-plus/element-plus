...
| resetFields                | Set initial values for form fields. When `resetFields` is called, fields will reset to these values.                                                                                                                                                                      | ^[Function]`(props?: Arrayable<FormItemProp> \| undefined) => void`                                                               |
...
| setInitialValues ^(2.13.1) | Set initial values for form fields. When `resetFields` is called, fields will reset to these values. All initialized fields (including those that have gone through mounted → unmounted) will be updated. Fields not defined in `initModel` will be reset to `undefined`. | ^[Function]`(initModel: Record<string, any>) => void`                                                                             |
