...
| count-graphemes ^(2.13.7)     | custom function to count graphemes; when set, native `maxlength`/`minlength` constraints are bypassed and this function controls the limit | ^[Function]`(value: string) => number`                                                              | —           |
