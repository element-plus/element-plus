...
:::demo `maxlength` and `minlength` attributes of input, they declare a limit on the number of characters a user can input. The "number of characters" is measured using JavaScript string length.Setting the `maxlength` prop for a text or textarea type of Input can limit the length of input value, allows you to show word count by setting `show-word-limit` to `true` at the same time. For textarea, You can set `show-word-limit` to `outside` to display the word count outside the textarea.
...
| show-word-limit             | whether show word count, only works when `type` is 'text' or 'textarea'. 'outside' and 'inside' are supported after the version ^(2.11.5) | ^[boolean] / ^[string]`'outside' \| 'inside'`                                                                                                                                                                                | false       |
