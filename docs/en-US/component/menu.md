...
| navigation-mode                 | strategy for router navigation - use the route's `path` or `name` when router is enabled" or similar, to clarify that this prop controls the navigation mode selection. | ^[enum]`'path' \| 'name'`              | path     |
