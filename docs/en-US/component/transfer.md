...
| target-order                | Order strategy for elements in the target list. `original`: keeps the same order as the data source; `push`: newly added elements are appended to the bottom; `unshift`: newly added elements are inserted at the top | ^[enum]`'original' \| 'push' \| 'unshift'`                         | original |
