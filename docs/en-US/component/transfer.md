...
| before-transfer ^(2.8.5) | the callback before the transfer, the parameter is the transferred data. If not return `true`, transfer will be aborted.                                                                                                                                                           | ^[Function]`(data: TransferKey[]) => Awaitable<boolean>`           | —        |
