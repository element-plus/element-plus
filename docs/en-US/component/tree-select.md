...
| setChecked ^(deprecated)             | set node to be checked or not, only works when `node-key` is assigned                                                | (key/data, checked, deep, ignoreCheckStrictly) Accept three parameters: 1. node's key or data to be checked 2. a boolean typed parameter indicating checked or not. 3. a boolean typed parameter indicating deep or not. 4. a boolean typed parameter indicating whether to ignore check-strictly or not. |
