...
describe('Calendar.vue', () => {
  it('create', async () => {
    ...
    const titleEl = wrapper.find('.g-calendar__title')
    ...
    const rows = wrapper.element.querySelectorAll('.g-calendar-table__row')
  ...
  it('range', () => {
    ...
    expect(
      wrapper.element.querySelector('.g-calendar__button-group')
    ).toBeNull()
  })
  ...
  it('firstDayOfWeek', async () => {
    ...
    const head = wrapper.element.querySelector('.g-calendar-table thead tr')
    expect(head?.firstElementChild?.innerHTML).toBe('Sun')
    expect(head?.lastElementChild?.innerHTML).toBe('Sat')
    const firstRow = wrapper.element.querySelector('.g-calendar-table__row')
  ...
  it('click previous month or next month', async () => {
    ...
    await nextTick()
    const btns = wrapper.findAll('.g-button')
  ...
  it('slots', async () => {
    ...
    expect(wrapper.find('.g-calendar__header').text()).toEqual(AXIOM)
  ...
  it('should work when controller-type is select', async () => {
    ...
    await nextTick()
    const selects = wrapper.findAllComponents({ name: 'GSelect' })
    const btn = wrapper.find('.g-button')
    const yearSelect = selects[0]
    const yearOptions = yearSelect.findAllComponents({ name: 'GOption' })
    const monthSelect = selects[1]
    const monthOptions = monthSelect.findAllComponents({ name: 'GOption' })
