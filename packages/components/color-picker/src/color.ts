...
export default class Color {
  ...
  fromString(value) {
    ...
    if (value.includes('hsl')) {
      ...
      if (parts.length === 4) {
        this._alpha = Math.floor(Number.parseFloat(parts[3]) * 100)
      ...
      if (!/^[\dA-Fa-f]{3}$|^[\dA-Fa-f]{6}$|^[\dA-Fa-f]{8}$/.test(hex)) return
      let r, g, b
      ...
      if (hex.length === 8) {
        this._alpha = Math.floor((parseHexChannel(hex.slice(6)) / 255) * 100)
