...
describe('Dialog.vue', () => {
  test('render test', async () => {
    ...
    await rAF()
    await nextTick()
    expect(wrapper.find('.g-dialog__body').text()).toEqual(AXIOM)
  })
  ...
  test('dialog should have a title and header when it has been given', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.g-dialog__header').text()).toBe(HEADER)
  ...
  test('dialog should have a footer when footer has been given', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.g-dialog__footer').exists()).toBe(true)
    expect(wrapper.find('.g-dialog__footer').text()).toBe(AXIOM)
  })
  ...
  test('should append dialog to body when appendToBody is true', async () => {
    ...
    await nextTick()
    expect(
      document.body.firstElementChild!.classList.contains('g-overlay')
    ).toBe(true)
    wrapper.unmount()
  })
  ...
  test('should center dialog', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.g-dialog--center').exists()).toBe(true)
  })
  ...
  test('should show close button', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.g-dialog__close').exists()).toBe(true)
  })
  ...
  test('should hide close button when showClose = false', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.g-dialog__headerbtn').exists()).toBe(false)
  })
  ...
  test('should close dialog when click on close button', async () => {
    ...
    await nextTick()
    await wrapper.find('.g-dialog__headerbtn').trigger('click')
  ...
  test('dialog content should not be clickable during close animation', async () => {
    ...
    expect(wrapper.find('.g-dialog').exists()).toBe(true)
    ...
    const overlayDialog = wrapper.find('.g-overlay-dialog')
  ...
  describe('mask related', () => {
    ...
    test('should close the modal when clicking on mask when `closeOnClickModal` is true', async () => {
      ...
      await nextTick()
      expect(wrapper.find('.g-overlay').exists()).toBe(true)
      expect(wrapper.find('.g-overlay-dialog').exists()).toBe(true)

      await triggerCompositeClick(wrapper.find('.g-overlay-dialog'))
      expect(wrapper.vm.visible).toBe(false)
    })
    ...
    test('should not click the mask to close when it is penetrable', async () => {
      ...
      const overlay = wrapper.findComponent({ name: 'GOverlay' })
      const dialog = wrapper.findComponent({ name: 'GDialog' })
  ...
  describe('life cycles', () => {
    ...
    test('should destroy on close', async () => {
      ...
      await rAF()
      await nextTick()
      expect(wrapper.find('.g-dialog__body').exists()).toBe(false)
    })
  ...
  describe('accessibility', () => {
    ...
    test('missing title attribute should point to header slot content', async () => {
      ...
      await nextTick()
      const dialog = wrapper.find('[role="dialog"]')
      const dialogTitle = wrapper.find('.g-dialog__title')
    ...
    test('aria-describedby should point to modal body', async () => {
      ...
      await nextTick()
      const dialog = wrapper.find('[role="dialog"]')
      const dialogBody = wrapper.find('.g-dialog__body')
