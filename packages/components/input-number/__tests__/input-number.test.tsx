...
describe('InputNumber.vue', () => {
  ...
  describe('overflow is round', () => {
    const num = ref(0)
    const wrapper = mount(() => (
      <InputNumber precision={3} overflow={'round'} v-model={num.value} />
    ))
    it.each([
      [1.11100001, '1.111'],
      [1.09, '1.090'],
      ...
      [1.2341, '1.234'],
      [10.9999, '11.000'],
    ])('each overflow property test: $input $output', async (input, output) => {
      await wrapper.find('input').setValue(input)
      expect(wrapper.find('input').element.value).toEqual(`${output}`)
    })
  })

  describe('overflow is ceil', () => {
    const num = ref(0)
    const wrapper = mount(() => (
      <InputNumber precision={3} overflow={'ceil'} v-model={num.value} />
    ))
    it.each([
      [1.11100001, '1.112'],
      [1.09, '1.090'],
  ...
  describe('overflow is floor', () => {
    const num = ref(0)
    const wrapper = mount(() => (
      <InputNumber precision={3} overflow={'floor'} v-model={num.value} />
    ))
    it.each([
      [1.11100001, '1.111'],
      [1.09, '1.090'],
  ...
  describe('overflow is custom function', () => {
    const num = ref(0)
    const precision = ref(0)
    const fn = (num: number, pre: number) => Number.parseFloat(num.toFixed(pre))
    const wrapper = mount(() => (
      <InputNumber
    ...
    it.each([
      ...
      [1.2341, 1],
      [10.9999, 0],
    ])('each overflow property test: $input $output', async (input, pre) => {
      precision.value = pre
      await nextTick()
      await wrapper.find('input').setValue(input)
      expect(num.value).toEqual(fn(input, pre))
    })
  })
})
