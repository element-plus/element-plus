...
describe('MessageBox', () => {
  ...
  test('create and close', async () => {
    ...
    await rAF()
    expect(
      msgbox.querySelector('.g-message-box__title span')?.textContent
    ).toEqual('消息')
    expect(
      msgbox.querySelector('.g-message-box__message')?.querySelector('p')
        ?.textContent
    ).toEqual('这是一段内容')
    /** custom inline style */
    expect(
      (msgbox.querySelector('.g-message-box') as HTMLElement).style.width
    ).toEqual('100px')
    MessageBox.close()
    await rAF()
  ...
  test('custom icon', async () => {
    ...
    await rAF()
    const icon = document.querySelector('.g-message-box__status')!

    expect(icon.classList.contains('g-icon')).toBe(true)
  ...
  test('html string', async () => {
    ...
    await rAF()
    const message = document.querySelector('.g-message-box__message strong')!
  ...
  test('alert', async () => {
    ...
    expect(msgbox.querySelector('.g-icon-warning')).toBeDefined()
  })
  ...
  test('confirm', async () => {
    ...
    const btn = document
      ...
      ?.querySelector('.g-button--primary') as HTMLButtonElement
    btn.click()
    await rAF()
    const msgbox: HTMLElement = document.querySelector(selector)!
  ...
  test('autofocus', async () => {
    ...
    const btnElm = document.querySelector(
      '.g-message-box__btns .g-button--primary'
    )!
  ...
  test('prompt', async () => {
    ...
    const inputElm = document
      .querySelector(selector)!
      .querySelector('.g-message-box__input')!
  ...
  test('prompt: should not confirm when pressing Enter during IME composition', async () => {
    ...
    const input = document
      .querySelector(selector)!
      .querySelector('.g-message-box__input input') as HTMLInputElement
    input.dispatchEvent(
  ...
  describe('promise', () => {
    ...
    test('reject', async () => {
      ...
      await rAF()
      const btn = document.querySelector('.g-message-box__btns .g-button')
  ...
  describe('context inheritance', () => {
    it('should globally inherit context correctly', () => {
      expect(GMessageBox._context).toBe(null)
      const testContext = {
        config: {
          globalProperties: {},
        },
      ...
      GMessageBox.install?.(testContext as any)
      expect(GMessageBox._context).not.toBe(null)
      expect(GMessageBox._context).toBe(testContext._context)
      // clean up
      GMessageBox._context = null
    })
  })
  ...
  describe('accessibility', () => {
    ...
    test('prompt inputValidator error message', async () => {
      ...
      const error = msgbox.querySelector('.g-message-box__errormsg')!
  ...
  test('should work with confirmButtonType and cancelButtonType props', async () => {
    ...
    const cancelBtn = document.querySelector(
      '.g-message-box__btns .g-button--warning'
    )
    const confirmBtn = document.querySelector(
      '.g-message-box__btns .g-button--danger'
    )
