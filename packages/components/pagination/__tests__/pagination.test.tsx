...
const assertCurrent = (wrapper: VueWrapper<any>, page: number) => {
  expect(wrapper.find('.g-pager li.is-active.number').text()).toBe(
    String(page)
  )
}
const assertPages = (wrapper: VueWrapper<any>, total: number) => {
  expect(wrapper.find('.g-pagination .g-pager li:last-child').text()).toBe(
    String(total)
  )
}
...
describe('Pagination', () => {
  describe('test invalid usages', () => {
    ...
    test('both absence of total & pageCount is invalid', async () => {
      ...
      expect(wrapper.find('.g-pagination').exists()).toBe(false)
      expect(console.warn).toHaveBeenCalled()
      total.value = 100
      await nextTick()
      expect(wrapper.find('.g-pagination').exists()).toBe(true)
    })
  ...
  describe('test layout & layout reactive change', () => {
    ...
    const layoutSelectorPairs = [
      ['sizes', '.g-pagination__sizes'],
      ['prev', 'button.btn-prev'],
      ['pager', 'ul.g-pager'],
      ['next', 'button.btn-next'],
      ['jumper', '.g-pagination__jump'],
      ['total', '.g-pagination__total'],
    ]
    ...
    test(`layout with '->, total'`, async () => {
      ...
      assertElementsExistence(
        wrapper,
        ['.g-pagination__total', '.g-pagination__rightwrapper'],
        true
      )
    })
    ...
    test('test small layout', () => {
      ...
      expect(wrapper.vm.$el.classList.contains('g-pagination--small')).toBe(
        true
      )
    })
  ...
  describe('test pageSize & currentPage reactive change', () => {
    ...
    test('test currentPage change', async () => {
      ...
      await wrapper.find('.g-pager li:last-child').trigger('click')
    ...
    test('test more icon', async () => {
      ...
      await wrapper.find('.g-pager li:nth-child(5)').trigger('click')
      assertCurrent(wrapper, 5)
      expect(wrapper.find('.btn-quickprev').exists()).toBeTruthy()
      expect(wrapper.findComponent(DArrowLeft).exists()).toBeFalsy()
      ...
      await wrapper.find('.g-pager li:nth-child(3)').trigger('click')
      assertCurrent(wrapper, 5)
      expect(wrapper.find('.btn-quicknext').exists()).toBeTruthy()
      expect(wrapper.findComponent(DArrowRight).exists()).toBeFalsy()
    ...
    test('test listener work', async () => {
      ...
      expect(count).toBe(1)
      await wrapper.find('.g-select').trigger('click')
      await wrapper
  ...
  describe('test a11y supports', () => {
    test('test a11y attributes', async () => {
      ...
      const prev = wrapper.find('.g-pagination .btn-prev')
      const next = wrapper.find('.g-pagination .btn-next')
      const pagers = wrapper.findAll('.g-pagination .g-pager .number')
    ...
    test('test tabindex interactive', async () => {
      const wrapper = mount(() => <Pagination total={100} />)
      await wrapper.find('.g-pager li:nth-child(2)').trigger('click')
      assertCurrent(wrapper, 2)
      await wrapper.find('.g-pager li:nth-child(3)').trigger('click', {
        key: 'Enter',
      })
      ...
      /*
       * await wrapper.find('.g-pager li:nth-child(3)').trigger('keyup', {
       *   key: 'Tab',
       * })
       * const style = window.getComputedStyle(wrapper.find('.g-pager li:nth-child(4)').element, ':focus-visible')
    ...
    test('test tabindex disabled', async () => {
      ...
      expect(
        wrapper.find('.g-pager li:first-child').attributes('tabindex')
      ).toBe('-1')
