<template>
  <div :class="{ 'has-seconds': showSeconds, [`${timePrefixClass}-spinner`]: true }">
    <template v-if="!arrowControl">
      ...
      >
        <li
          ...
          :key="key"
          :class="{ active: key === timePartsMap[item].value, disabled, [`${timePrefixClass}-spinner__item`]: true }"
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const adjustSpinner = (type, value) => {
      ...
      if (el.value) {
        el.value.$el.querySelector(`.${scrollbarPrefixClass.value}__wrap`).scrollTop = Math.max(
          0,
    ...
    const handleScroll = (type) => {
      ...
      const value = Math.min(
        Math.round(
          (listRefsMap[type].value.$el.querySelector(`.${scrollbarPrefixClass}__wrap`)
            .scrollTop -
