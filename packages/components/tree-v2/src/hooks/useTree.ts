...
export function useTree(props: ITreeProps, emit) {
  ...
  const tree = computed<Tree>(() => {
    ...
    // Tree level and the nodes of that level
  ...
  const childrenKey = computed(() => {
    return (props.props && props.props.children) || 'id'
  })
  const disabledKey = computed(() => {
    return (props.props && props.props.disabled) || 'id'
  })
  const labelKey = computed(() => {
    return (props.props && props.props.label) || 'id'
  })
