...
import { Table, AnyObject, TableColumn } from '../table'
...
function useStyles(props: TableBodyProps) {
  ...
  const getRowClass = (row: AnyObject, rowIndex: number) => {
    ...
    if (parent.ctx.highlightCurrentRow && row === props.store.states.currentRow.value) {
  ...
  const getCellStyle = (rowIndex: number, columnIndex: number, row: AnyObject, column: TableColumn) => {
  ...
  const getCellClass = (rowIndex: number, columnIndex: number, row: AnyObject, column: TableColumn) => {
  ...
  const getSpan = (row: AnyObject, column: TableColumn, rowIndex: number, columnIndex: number) => {
    let rowspan = 1
    let colspan = 1
  ...
  const getColspanRealWidth = (columns: TableColumn[], colspan: number, index: number) => {
    if (colspan < 1) {
