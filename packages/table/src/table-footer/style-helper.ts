...
import { Table, TableFooter, TableColumn } from '../table'
...
function useStyle(props: TableFooter) {
  ...
  const isCellHidden = (index: number, columns: TableColumn[], column: TableColumn) => {
  ...
  const getRowClasses = (column: TableColumn, cellIndex: number) => {
