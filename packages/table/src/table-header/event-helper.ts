...
function useEvent(props: TableHeaderProps, emit) {
  ...
  const handleFilterClick = (event: Event, column: TableColumn) => {
    event.stopPropagation()
    return
  }

  const handleHeaderClick = (event: Event, column: TableColumn) => {
  ...
  const handleHeaderContextMenu = (event: Event, column: TableColumn) => {
    parent.emit('header-contextmenu', column, event)
  }
  const draggingColumn = ref(null)
  const dragging = ref(false)
  const dragState = ref({})
  const handleMouseDown = (event: MouseEvent, column: TableColumn) => {
    if (isServer) return
  ...
  const handleMouseMove = (event: MouseEvent, column: TableColumn) => {
    if (column.children && column.children.length > 0) return
  ...
  const handleSortClick = (event: Event, column: TableColumn, givenOrder: string | boolean) => {
    event.stopPropagation()
    const order =
