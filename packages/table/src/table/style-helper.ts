...
import { TableProps, Table, AnyObject, TableLayout, Store, TableColumn, fn } from '../table'

function useStyle(props: TableProps, layout: TableLayout, store: Store, table: Table, doLayout: fn) {
  ...
  onMounted(() => {
    ...
    // init filters
    store.states.columns.value.forEach((column: TableColumn) => {
  ...
  const bodyWidth = computed(() => {
    ...
    return bodyWidth_.value ? bodyWidth_.value as number - (scrollY.value ? gutterWidth : 0) + 'px' : ''
  })
