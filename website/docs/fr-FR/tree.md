...
| setCurrentKey   | Met un noeud en valeur par sa clé, ne marche que si `node_key` est assigné. | (key, shouldAutoExpandParent) 1. la clé du noeud. Si `null`, annule la sélection actuelle. 2. whether to automatically expand parent node |
| setCurrentNode  | Met un noeud en valeur, ne marche que si `node_key` est assigné. | (node, shouldAutoExpandParent) 1. le noeud. 2. whether to automatically expand parent node |
